...
## 二十四、第七波 7e：全树重路由异步分片 + 新弹合并

- Tree v73：v68 全树刷新不再在一个 tick 内跑完。每 tick 最多处理 6 个父层；完成前旧 commitNode 继续执行，不阻塞游戏。
- 分片期间再来新弹：直接丢弃当前分片任务并以最新 threats 重启；已处理节点因签名已刷新会被自动跳过，不会重复计算。旧任务不会叠加。
- 子弹消失会作废旧分片，增删混合交给后续惰性/提交刷新，保持原语义。
- 重路由完成时才统一重选所有 next + 回传 subtreeBest。

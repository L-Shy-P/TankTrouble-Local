/**
 * 逆杀戮场（inverse killfield）—— 从 killfield-main/src/killfield/field.js 拷贝而来。
 * 只改了一行 import（指到我们的 ./constants.js），算法本体一字未动。
 * 我们的 gameView（见 gameView.js）提供它需要的 5 样东西：
 *   walls（墙的 AABB 列表）、wallHalfT（墙半厚）、reachable（可达格）、
 *   maze（二维数组，只取尺寸）、scale（一格的世界尺寸），外加 wallHit(x,y)。
 */
/**
...
import * as C from "./constants.js";   // 我们的常量表（主人定的规矩：先拷贝再改）

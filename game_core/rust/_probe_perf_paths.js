...
async function main() {
  ...
  // ===== 提交切片改造后的净收益：Rust 全包 + 选中节点 JS 确认 =====
  (function commitSliceNet() {
    const N = 12;
    console.log('');
    console.log('=== 提交切片一次的总耗时：改造前 vs 改造后 ===');
    for (let si = 0; si < 2; si++) {
      const prep = prepared[si];
      const adapter = prep.adapter, threats = prep.threats;
      const one = [ops[1]];
      // 改造前：9 操作 JS scorePaths
      let t0 = performance.now();
      for (let i = 0; i < N; i++) VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, ops, frames, threats, cfg);
      const before = (performance.now() - t0) / N;
      // 改造后：1 次 Rust 全包（9 操作） + 选中节点的 JS 融合确认（1 操作 JS）
      t0 = performance.now();
      for (let i = 0; i < N; i++) adapter.simulateTankBatchScored(startPose, ops, frames, { startPose, threats, tGlobal: 0, cfg });
      const rust = (performance.now() - t0) / N;
      t0 = performance.now();
      for (let i = 0; i < N; i++) VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, one, frames, threats, cfg);
      const confirm = (performance.now() - t0) / N;
      console.log('  ' + prep.scene.name.padEnd(18) + ' 改造前 ' + before.toFixed(2) + 'ms   改造后 ' +
        rust.toFixed(2) + 'ms + 确认 ' + confirm.toFixed(2) + 'ms = ' + (rust + confirm).toFixed(2) + 'ms   ' +
        (before / Math.max(0.001, rust + confirm)).toFixed(1) + 'x');
    }
  })();

  // ===== 决定性验证：Rust 生长路径认不认 k / 动作成本 =====
  (function checkK() {
    const prep = prepared[0];
    const adapter = prep.adapter, threats = prep.threats;
    const base = adapter.simulateTankBatchScored(startPose, ops, frames, { startPose, threats, tGlobal: 0, cfg: cfg });
    const withK = adapter.simulateTankBatchScored(startPose, ops, frames, {
      startPose, threats, tGlobal: 0,
      cfg: Object.assign({}, cfg, { safeFilterK: 0.4, actionCostPerFrame: 3, occlusionEnabled: true })
    });
    if (!base || !withK) { console.log('k 验证：路径不可用，跳过'); return; }
    let sameScores = true, sameTotal = true;
    for (let i = 0; i < base.length; i++) {
      if (base[i].totalScore !== withK[i].totalScore) sameTotal = false;
      for (let f = 0; f < base[i].perFrameScores.length; f++) {
        if (base[i].perFrameScores[f] !== withK[i].perFrameScores[f]) { sameScores = false; break; }
      }
    }
    console.log('');
    console.log('=== 验证：生长路径（Rust）认不认 k=0.4 / 动作成本=3 ===');
    console.log('  逐帧分数完全相同 : ' + sameScores);
    console.log('  总分完全相同     : ' + sameTotal);
    console.log('  结论：' + (sameScores && sameTotal
      ? 'Rust 生长路径完全忽略 k / 动作成本（k 只对 JS 路径生效）'
      : 'Rust 生长路径受 k / 动作成本影响'));
    console.log('  对照：JS 评分路径对同一局面 k=1 vs k=0.4：');
    const jsBase = VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, ops, frames, threats, cfg);
    const jsK = VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, ops, frames, threats,
      Object.assign({}, cfg, { safeFilterK: 0.4 }));
    console.log('    总分：k=1 → ' + jsBase.map(r => r.totalScore.toFixed(1)).join(','));
    console.log('           k=0.4 → ' + jsK.map(r => r.totalScore.toFixed(1)).join(','));
  })();

  // ===== 临时计时探针：JS scorePaths（真适配器 = Rust物理 + JS评分） vs Rust 全包 =====
  (function timing() {
    const N = 12;
    console.log('');
    console.log('=== 单次「9 操作 × 75 帧」耗时（同一局面，真适配器） ===');
    console.log('  场景                 JS评分路径      Rust全包     倍数');
    for (let si = 0; si < prepared.length; si++) {
      const prep = prepared[si];
      const adapter = prep.adapter, threats = prep.threats;
      try {
        VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, ops, frames, threats, cfg);
        adapter.simulateTankBatchScored(startPose, ops, frames, { startPose, threats, tGlobal: 0, cfg });
      } catch (eWarm) { console.log('  预热失败 ' + prep.scene.name + ': ' + eWarm.message); continue; }
      let t0 = performance.now();
      for (let i = 0; i < N; i++) {
        VantageScoring.scorePaths(adapter, { tank: startPose, tGlobal: 0 }, ops, frames, threats, cfg);
      }
      const jsMs = (performance.now() - t0) / N;
      t0 = performance.now();
      for (let i = 0; i < N; i++) {
        adapter.simulateTankBatchScored(startPose, ops, frames, { startPose, threats, tGlobal: 0, cfg });
      }
      const rustMs = (performance.now() - t0) / N;
      console.log('  ' + prep.scene.name.padEnd(18) + ' ' +
        jsMs.toFixed(2).padStart(8) + 'ms ' + rustMs.toFixed(2).padStart(9) + 'ms ' +
        (jsMs / Math.max(0.001, rustMs)).toFixed(1).padStart(7) + 'x');
    }
  })();

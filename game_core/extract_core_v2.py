...
def get_cache_path(url):
    """根据URL生成本地缓存路径（与proxy_server.py相同）"""
...
def extract_game_core():
    ...
    os.makedirs("RELEASE-2026-05-11-01/assets", exist_ok=True)
    ...
    for url, dst in jquery_files:
        cache_path = get_cache_path(url)
        if os.path.exists(cache_path):
            shutil.copy2(cache_path, dst)
            print(f"[OK] jQuery: {os.path.basename(dst)}")
        else:
            print(f"[MISS] jQuery: {url}")
    ...
    for url, dst in jquery_css:
        cache_path = get_cache_path(url)
        if os.path.exists(cache_path):
            shutil.copy2(cache_path, dst)
            print(f"[OK] jQuery CSS: {os.path.basename(dst)}")
    ...
    for url in script_urls:
        if "cdn.tanktrouble.com" in url:
            ...
            if os.path.exists(cache_path):
                ...
                target_path = os.path.join("RELEASE-2026-05-11-01/js", filename)
                shutil.copy2(cache_path, target_path)
                cdn_js_count += 1
    
    print(f"[OK] CDN JS文件: {cdn_js_count}个")
    ...
    for url in css_urls:
        if "cdn.tanktrouble.com" in url:
            ...
            if os.path.exists(cache_path):
                filename = os.path.basename(url.split('?')[0])
                target_path = os.path.join("RELEASE-2026-05-11-01/css", filename)
                shutil.copy2(cache_path, target_path)
                css_count += 1
    
    print(f"[OK] CSS文件: {css_count}个")
        ...
        for file in files:
            if any(file.lower().endswith(ext) for ext in img_extensions):
                ...
                if "cdn_tanktrouble_com" in root:
                    target_path = os.path.join("RELEASE-2026-05-11-01/assets", file)
                    shutil.copy2(src_path, target_path)
                    img_count += 1
    
    print(f"[OK] 图片资源: {img_count}个")

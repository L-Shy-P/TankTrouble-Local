...
(function(global) {
    ...
    var DANGER_RULES = {
        ...
        // v102：这三项目前都"没接完"，改动它们只有坏处（退回 JS 评分变慢 /
        // 行为口径不统一），所以按主人的要求与其它危险开关一样显红。
        'exp-occlusion': function(e) {
            return e.occlusion === false
                ? '关掉角度遮蔽评分：AI 失去唯一的威胁梯度（有子弹也不会躲）；v116 起该开关已接进 Rust，不再拖性能，但仍只用于对比实验'
                : null;
        },
        'exp-safeFilterK': function(e) {
            return Number(e.safeFilterK) < 1
                ? '开安全过滤 k：评分退回 JS 路径（帧率明显下降），而且生长路径走 Rust 不认 k —— 口径混合、结论不可信'
                : null;
        },
        'exp-actionCost': function(e) {
            return Number(e.actionCost) > 0
                ? '开动作成本：实战走的批量评分分支里这项尚未实现（不改变 AI 行为），只会退回 JS 路径拖慢帧率'
                : null;
        }
    };
    ...
    function ensurePanel() {
        ...
        (function bindExpTooltips() {
            ...
            var tips = {
                ...
                'exp-occlusion': '角度遮蔽评分（默认开）。它是唯一衡量“子弹威胁”的项：子弹离坦克越近，被遮蔽的安全角度就越多、分越低。关掉它就只剩地形/杀戮场引导——**有子弹时AI会失去躲弹梯度**，只建议做对比实验用。**当前状态（v116）：该开关已接进 Rust（ABI v7），开关它都不再掉性能；但它仍是唯一的威胁梯度，关掉有子弹就不会躲——只用于对比实验，所以改成非默认值仍会显红。**',

                'exp-safeFilterK': '安全过滤阈值 k（0.10~1.00）。设单帧遮蔽满分 a=(2π)²，本帧分 b，则用 b′ = min(a·k, b)：把“已经足够安全”的帧一律压成同一个值，于是“刚好够安全”和“非常安全”等价，AI 不再为了更远而大幅转向；危险区间（b<a·k）**保留完整梯度**，所以贴近子弹时该有的精细躲法不受影响。k=1 不截顶（原行为），越小压得越平。实测：遮蔽分在约 3.05 米处是断崖（9.4 → 39.5 满值），所以 k=0.3~0.5 正好把“跨过那条线”压到与“远远躲开”等价，又不动 3 米内的梯度——先从 0.4 试。 **当前状态（v102）：k 还没接进 Rust（生长路径不认它），打开会让评分退回 JS 路径、帧率明显下降，测出来的行为是混合口径——先别当结论用；所以现在改成非默认值会显红。**',
                ...
                'exp-actionCost': '动作成本（分/帧，0=关）。**只在“这一帧已经完全安全”时计**：转向和移动各算 1 分强度，同时踩两者算 2。危险帧不计，保证躲弹时不被拖后腿；安全帧才用它区分“谁更省事”，让 AI 别做多余动作。建议从 2~4 试。 **当前状态（v102）：实战走的“批量评分分支”里这项还没实现（AI 行为不受影响），唯一的实际效果是退回 JS 路径拖慢帧率——先别开；所以现在改成非默认值会显红。**',
    ...
    function handlePanelAction(act, ds, srcEl) {
        ...
        if (act === 'exp-safeFilterK') {
            state.exp.safeFilterK = Math.max(10, Math.min(100, parseInt(srcEl.value, 10) || 100)) / 100;
            if (typeof VantageTree !== 'undefined' && VantageTree.setSafeFilterK) {
                VantageTree.setSafeFilterK(state.exp.safeFilterK);
            }
            updatePanel();
            return;
        }
    ...
    function syncSliderActivity() {
        ...
        dimGroup([_expCtrl.safeFilterK, _expCtrl.actionCost], state.exp.occlusion === false);
    ...
    function syncExpControls() {
        ...
        if (_expCtrl.safeFilterKSpan) {
            var kTxt = (state.exp.safeFilterK || 1).toFixed(2);
            if (_expCtrl.safeFilterKSpan.textContent !== kTxt) _expCtrl.safeFilterKSpan.textContent = kTxt;
        }
        if (_expCtrl.actionCost) {

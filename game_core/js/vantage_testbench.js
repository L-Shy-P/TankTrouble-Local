    ...
    var DANGER_RULES = {
        ...
        'exp-occlusion': function(e) {
            return e.occlusion === false
                ? '关掉角度遮蔽评分：AI 失去唯一的威胁梯度（有子弹也不会躲），且评分退回 JS 路径、帧率明显下降；只用于对比实验'
                : null;
        },
    ...
    function ensurePanel() {
        ...
        (function bindExpTooltips() {
            ...
            var tips = {
                ...
                'exp-occlusion': '角度遮蔽评分（默认开）。它是唯一衡量“子弹威胁”的项：子弹离坦克越近，被遮蔽的安全角度就越多、分越低。关掉它就只剩地形/杀戮场引导——**有子弹时AI会失去躲弹梯度**，只建议做对比实验用。**当前状态（v102）：改动它会退回 JS 评分路径、帧率明显下降，所以现在改成非默认值会显红。**',
